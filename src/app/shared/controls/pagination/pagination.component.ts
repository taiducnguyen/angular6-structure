...
export class PaginationComponent implements ControlValueAccessor {
    ...
    public constructor( @Inject(PaginationConfig) paginationConfig: PaginationConfig) {
        if (!this.config) {
    ...
    //
    public ngOnChanges() {
        if ((this._currentTotalItems !== this.totalItems) || (this.currentPageIndex && this.page !== this.currentPageIndex)) {
